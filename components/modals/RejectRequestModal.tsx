...
const RejectRequestModal = ({ onClose, requestId ,requestMakerId ,sheetId,resourceId }: { onClose: () => void, requestId: number ,requestMakerId:number,sheetId?:number,resourceId?:number }) => {
  const handleAccept = () => {
    if (resourceId) {
      axios.post(`/api/request/reject`, {
        requestId,
        requestMakerId,
        resourceId
        
       }).then(()=>toast.success('Request Rejected')).catch((err)=>toast.error(err.response.data)).finally(()=>onClose())
    }
    if (sheetId) {
  ...
  return (
    <div className='fixed inset-0 bg-black/40 z-40 flex flex-row items-center justify-center'> 
      <div className='bg-white p-4 rounded max-w-md w-full flex flex-col gap-2 relative'>
        ...
        <SubHeading body='Reject Request ???' className='text-center' />
