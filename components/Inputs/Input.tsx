...
const Input: React.FC<InputProps> = ({ label, type = "text", id, register, required, errors, isLoading ,className,placeholder }) => {
  ...
  return (
    <div className={clsx('flex flex-col w-4/5 gap-2',className)}>
          ...
          form-input block rounded-md border-0 px-6 py-2   shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset bg-white/10 text-blue-500 focus:ring-sky-600 lg:text-lg xs:text-sm font-medium `,
