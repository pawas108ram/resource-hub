...
const FolderEditingModal = ({ folder, onClose,question }: { folder: Folder & {questions:Question[]}, onClose: () => void ,question:FullQuestionType[]}) => {
  ...
  return (
    <div className='fixed  bg-black/40 flex flex-row items-center justify-center z-[99999] inset-0'>
      <form onSubmit={ (e)=>handleSubmit(e)} className='bg-black text-white p-4 rounded xs:w-4/5 lg:w-2/5 relative flex flex-col gap-4 '>
              ...
              <label htmlFor="title" className='text-2xl font-bold'>Title</label>
                    <input type="text" name="title" id="title" placeholder='Enter title for Folder...' className='form-input rounded bg-white/10 text-white' disabled={isLoading} onChange={(e)=>setTitle(e.target.value)} value={title} />
                    <label htmlFor="description" className='text-2xl font-bold'>Description</label>
              <textarea name="description" id="description" placeholder='Enter description for Folder...' className='form-textarea resize-none bg-white/10 text-white' disabled={isLoading} onChange={(e) => setDescription(e.target.value)} value={description} rows={5} cols={5} />
