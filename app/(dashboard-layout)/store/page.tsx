...
const StorePage = () => {
  ...
  return (
      <div className='bg-black/90 w-full max-h-screen text-white flex flex-col items-center p-4 gap-1 overflow-hidden xs:p-2 xs:pb-32 lg:pb-0 '>
          {currentUser !== null ? <> <Heading body='ResourceHub Store' className='text-white underline' />
              <div className='flex flex-row  gap-2 items-center justify-end p-2 w-full  '>
            ...
            <span className='   font-semibold flex flex-row items-center gap-2   '><FaCoins size={20} /> {currentUser.coins} coins</span>
             ...
             <div className='grid lg:grid-cols-4 xs:grid-cols-2  gap-2  xs:w-full lg:w-5/6 bg-white/10 rounded p-3 h-full overflow-y-auto  '>
